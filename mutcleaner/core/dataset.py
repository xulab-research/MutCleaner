...
class MutationDataset:
    ...
    def convert_codon_to_amino_acid_sets(self, convert_labels: bool = False) -> "MutationDataset":
        ...
        for ref_id, sequence in self.reference_sequences.items():
            if isinstance(sequence, (DNASequence, RNASequence)):
                ...
                converted_dataset.add_reference_sequence(
                    ref_id,
                    translated_sequence,
                )
    ...
    def save_by_reference(self, base_dir: Union[str, Path]) -> None:
        ...
        for ref_id, data in tqdm(
            ...
            sequence_name = ref_sequence.name if ref_sequence.name else str(ref_id)
    ...
    def from_dataframe(
        ...
        if df["mutation_set_id"].notna().all() and df["mutation_set_id"].is_unique:
    ...
    def _reconstruct_unique_mutation_sets(
        ...
        for values in tqdm(rows, total=len(df), desc="Reconstructing mutation sets"):
            set_name = values[set_name_index] if set_name_index is not None else None
